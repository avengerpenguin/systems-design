...
\begin{figure}[p]
  \begin{center}
    \includegraphics[height=0.4\textheight]{sequence_user.png}
  \end{center}
...
\begin{figure}[p]
  \begin{center}
    \includegraphics[height=0.4\textheight]{sequence_ajax.png}
  \end{center}
...
\section{Discussion and Appraisal}
\label{appraisal}

The first question that should be asked is whether the problem
given has been solved as much as it can be. The problem of
...
very little has been touched on around user's seeking educational
...
indirectly getting their content into the seach system demonsrates
the need for asynchronous messaging (and publish-subscribe), but
the nuances of such a process are not comphrehensively shown in
...
(and thus avoids any synchronisation issues if an item is updated as
source).

The latter approach fully decouples the search indexes from any
deeper information -- leaving that responsbility to appropriate
source system -- but means that the raw, stub results are near useful
...
actual information?). Given the AJAX-driven behaviour appears
...
without the need to maintain more than is necessary. The use
of \emph{subtype polymorphism} \cite{booch2007object} is likely
to be key to ensure the search domain model needs only use the APIs
...
Some of the advantages from duck-typing could be mirrored in
providing building the domain model around interfaces -- e.g.
\emph{class FootballMatch extends Event implements WeatherLocation} --
but it is still cumbersome to create such a large, static-typed
domain model that is used both at index time (in extracting the
information to build the indexes) and at query time (to understand
what item we have found and how to present it to the user).
...
architecture around performance and resilience. It is perhaps
...
in parts of the system to tolerate failure of certain components (e.g.
...
iteration of a project in an \emph{Agile} methodology. \cite{beck2001manifesto}
...
information to answer that (at which point we would hope it is as comprehensive
as is necessary to support the application currently running and no more).
...
communication aspect UML modelling and argue that encouraging such debate
...
Some of this hints at drawback of UML modelling being that it encourages
a lot of design decision at an early stage -- stage at which we arguably
have the least information. \cite{kelly2013conway} There are plenty of
...
Rational Unified Process (RUP) and Agile Unified Process (AUP) are described by
Ambler. \cite{ambler2002agile} It may have been more suitable to incorporate
